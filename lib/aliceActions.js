...
function aliceDiscovery(message, callback) {
  ...
  homebridge.HAPaccessories(function(endPoints) {
    ...
    // Dumb way to get all current states
    var states = hbDevices.homebridges.flatMap((hb) =>
      Object.values(hb.accessories).flatMap((acc) =>
        Object.values(acc.services).flatMap((service)=> 
          Object.values(service.characteristics).filter(char => char.capabilities.length > 0).flatMap((characteristic) =>
            new Promise((resolve, reject) => 
              homebridge.HAPstatus(service.host, service.port, `?id=${characteristic.aid}.${characteristic.iid}`, (err, value) => { 
                if(!err) characteristic.value = value.characteristics[0].value;
                resolve(true);
              })
            )
          )
    ...
    Promise.all(current_states).then(() => callback(null, response) );
  }.bind(this));
}
      ...
      for(var i = 0; i < message.payload.devices.length; i++) {
        ...
        for(var j = 0; j < device_data.capabilities.length; j++) {
          ...
          if (cap_status.error_code) {
            ...
            debug(cap_status.error_message, capability_data.type);
  ...
  try {
    ...
    homebridge.HAPaccessories(function(endPoints) {
      ...
      for(var i = 0; i < message.devices.length; i++) {
        ...
        response.payload.devices.push(dev_status.service.getDeviceState(homebridge));
      }

      Promise.all(response.payload.devices).then((result) => {
        response.payload.devices = result.flat();
        callback(null, response);
      });
    }.bind(this));
  } catch(e) {
